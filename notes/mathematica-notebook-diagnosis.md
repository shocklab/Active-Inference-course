...
Re-verified symbolically in Mathematica on 2026-08-27, since this is the claim
the whole diagnosis rests on: if the free energy were wrong, the fault would be
algebra rather than approximation and everything below would be beside the point.
`Expectation` over `ExponentialDistribution` reproduces both expressions and
`Simplify[difference] === 0` on each. Script and gate log in
`notes/diag.wls`, six gates, all pass.
...
is exactly 1, whatever the rate; the true posterior's is **0.71**, more
concentrated than any exponential can be.

The rigidity is worse than that number suggests, and Mathematica gives the sharp
form of it. Because the likelihood involves `s1^2` and `s2^2`, the free energy
carries `E[(s2 s1^2)^2] = 48/(a^4 b^2)` against a squared mean of `4/(a^4 b^2)`.
So the second moment is **twelve times** the square of the mean, and therefore
the *variance* of the predicted observation is **eleven times** its squared mean:
the family insists the prediction has a coefficient of variation of `sqrt(11)`,
about 3.3, whatever its parameters are set to.

That is the whole of cause 1. The agent cannot say "the observation is near 4 and
I am fairly sure"; the only sentence available is "the observation is near 4,
give or take 13". With one parameter, the sole way to shrink the implied
predictive spread is to shrink the mean, and the optimiser does exactly that.
...
`lambda <- 1 / mean(E_q[s])`, which is the *harmonic* mean of the rates. Mathematica
confirms `Reduce[ForAll[...], AM >= HM] === True` over the positive reals, so the
bias has a direction and it is upward: at rates 0.9 and 3.0 the arithmetic mean
exceeds the harmonic by a factor of 1.41. Arithmetic mean of rates >= harmonic mean by
